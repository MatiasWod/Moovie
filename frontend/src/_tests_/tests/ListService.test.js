...
describe('ListService', () => {
  ...
  test('should create a new moovie list', async () => {
    ...
    const result = await ListService.createMoovieList({
      name: 'New List',
      type: MoovieListTypes.MOOVIE_LIST_TYPE_DEFAULT_PUBLIC,
      description: 'Description',
    });

    expect(listApi.createMoovieList).toHaveBeenCalledWith(
      'New List',
      MoovieListTypes.MOOVIE_LIST_TYPE_DEFAULT_PUBLIC,
      'Description'
    );
  ...
  test('should insert media into a moovie list', async () => {
    ...
    expect(listApi.insertMediaIntoMoovieList).toHaveBeenCalledWith({
      id: 1,
