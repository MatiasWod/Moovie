  ...
  const createListCallback = async () => {
    try {
      const response = await ListService.createMoovieList({
        name: name,
        type: isPrivate
          ? MoovieListTypes.MOOVIE_LIST_TYPE_STANDARD_PRIVATE.type
          : MoovieListTypes.MOOVIE_LIST_TYPE_STANDARD_PUBLIC.type,
    ...
    <div className={'d-flex flex-column'}>
      ...
      <div className={'d-flex flex-row m-2'}>
        ...
        <div className={'container d-flex flex-column'}>
          <div
            style={{ overflowY: 'auto', maxHeight: '80vh', width: '60vw' }}
            className={'flex-wrap d-flex justify-content-evenly'}
          >
            {mediasLoading ? (
              <Spinner />
            ) : (
              medias.data.map((media) => (
                <MediaCard
                  ...
                  media={media}
                  onClick={() => onClickCallback(media)}
                />
              ))
            )}
            {mediasError && (
              <div>
                {t('createList.general')}: {mediasError.message}
              </div>
            )}
          </div>
        ...
        <div style={{ maxWidth: '22vw' }} className={'container d-flex flex-column'}>
          <CreateListForm
            ...
            isPrivate={isPrivate}
            setIsPrivate={(value) => dispatch(setIsPrivate(value))}
