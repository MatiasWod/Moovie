...
function List() {
  ...
  useEffect(() => {
    async function getWatchedCount() {
      try {
        const data = await profileService.getWatchedCountFromMovieListId(
          id,
          user.username
        );
  ...
  const [listRecommendationsLoading, setlistRecommendationsLoading] =
    useState(true);
  const [listRecommendationsError, setlistRecommendationsError] =
    useState(null);
  ...
  if (listLoading || listContentLoading || listRecommendationsLoading)
    return (
    ...
    <div className="default-container moovie-default">
      ...
      {isLoggedIn && list && list.data && (
        <div style={{ marginTop: "5px" }}>
          <ProgressBar
            now={
              list.data.mediaCount === 0
                ? 100
                : (watchedCount / list.data.mediaCount) * 100
            }
            label={
              t("profile.watched") +
              ": " +
      ...
      {listContentLoading ? (
        ...
        <ListContentPaginated
          ...
          isOwner={
            isLoggedIn === true && list?.data.createdBy === user.username
          }
          listId={id}
          Refresh={Refresh}
        />
