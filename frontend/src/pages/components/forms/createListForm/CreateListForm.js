      ...
      <Card.Body>
        ...
        {showSuccess && (
          <Alert variant="success" dismissible onClose={() => setShowSuccess(false)}>
            {t('createListForm.successMessage')}{' '}
            <Link to={'/list/' + listId}>{t('createListForm.link')}</Link>
          </Alert>
        )}
        <div className={'d-flex flex-row justify-content-between align-items-center'}>
          <Card.Title className="text-center">
            ...
            <OverlayTrigger
              placement="top-end"
              overlay={<Tooltip>{t('createListForm.tooltip')}</Tooltip>}
            >
          ...
          <div style={{ maxHeight: '35vh', overflowY: 'auto' }}>
            {selectedMedia.map((media) => (
              <Button
                key={media.id}
                variant="light"
                className="w-100 mb-2 d-flex flex-row align-items-center justify-content-between"
                onClick={() => onDeleteCallback(media)}
              >
