...
const ReviewItem = ({ review, source, isLoggedIn, currentUser, handleReport, reloadReviews }) => {
  ...
  return (
    <div key={review.id} className="review container-fluid bg-white my-3">
        ...
        <div>
          <div>
            ...
            {isLoggedIn && review.username === currentUser.username && (
              <button className="btn btn-primary btn-sm me-1" onClick={handleToggleEdit}>
                <i className="bi bi-pencil"></i> Edit
              </button>
            )}
            {isLoggedIn && review.username === currentUser.username && (
              <button className="btn btn-danger btn-sm" onClick={handleToggleDelete}>
      ...
      {showDeleteReview && (
        <div className="overlay">
          <ConfirmationModal
            ...
            message={t('reviews.aboutToDelete')}
      ...
      {showReportForm && (
        <ReportForm onReportSubmit={handleReportSubmit} onCancel={() => setShowReportForm(false)} />
      )}
    </div>
  );
};
