...
public class DirectorHibernateDao implements DirectorDao{
    ...
    public List<Director> getDirectorsForQuery(String query, int size) {
        String sql = "SELECT new ar.edu.itba.paw.models.Cast.Director(d.directorId,d.director,(SELECT COUNT(m) FROM Movie m WHERE m.directorId = d.directorId)) FROM Movie d WHERE LOWER(d.director) LIKE :query GROUP BY d.directorId,d.director ORDER BY d.director DESC";
